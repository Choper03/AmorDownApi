...
const SesionesDisponibles = async (req, res) => {
    ...
    connection.query('SELECT S.ID_SESION, S.NUMERO_SESION FROM SESIONES S WHERE NOT EXISTS ( SELECT 1 FROM SESIONES_BENEFICIARIO SB WHERE SB.ID_SESION = S.ID_SESION AND SB.ID_BENEFICIARIO = ? AND SB.FECHA = ? )', [Beneficiario, Fecha], (error, results) => {
        if(error){
            console.log(error);
